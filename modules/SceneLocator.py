...
import cv2
import numpy as np
from config import yolo_model, class_names, yolo_device
...
def FindObjects(img, target_object_depth_val, monocular_depth_val):
    ...
    if bool(target_object_depth_val):
        ...
        for box in boxes:
            ...
            cv2.putText(img, f'{class_names[cls_id].upper()}- {output_face/1000:.1f}m', (x, y - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (safety[1]), 3)
