...
class PriorityAnalyzer:
    ...
    def analyze(self, detections_summary, img_width=640):
        """
        ...
        # Priority 1: 交通信号灯检测
        traffic_light_alert = self._check_traffic_lights(detections_summary)
    ...
    def _check_traffic_lights(self, detections):
        """
        检测红绿灯状态变化或接近警告
        只在以下情况报告：
        1. 状态变化（红→绿或绿→红）
        ...
        for d in detections:
            ...
            if any(keyword in label_lower for keyword in traffic_light_keywords):
                ...
                if self.debug:
                    print(f"[DEBUG] Found traffic light: {d['label']} at {d['distance_m']:.1f}m")
        ...
        if 'r_signal' in label or 'r_light' in label or 'red' in label:
            current_state = 'red'
            state_text = "RED LIGHT"
        elif 'g_signal' in label or 'g_light' in label or 'green' in label:
            current_state = 'green'
